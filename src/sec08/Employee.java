	...
	public static void main(String[] args) {
		// 기초 데이터 만들기(배열 변수)
			...
			for (int i = 0; i < name.length; i++) {
				System.out.println(name[i] + "\t" + age[i] + "\t" + phone[i] + "\t" + gender[i]  
											+ "\t" + pay[i] + "\t" + country[i]  );
			}
			System.out.println("=======================================================");
			...
			int counta = 0;			// 사원수(카운트 초기화)
			double avgPay = 0;		// 여사원 평균 급여
			char v = 'F';			// 성별 여자
			
			// 급여와 인원수 누적
			for (int i = 0; i < gender.length; i++) {
				if (gender[i] == v) {	// 성별이 여자
					sumPay += pay[i];	// 급여 누적
					counta++;			// 인원수 누적
				}
			}
			avgPay = (double)sumPay / counta;	// 여사원 평균 급여 계산
			System.out.printf("3. 여자 사원의 숫자는 : %d명 이며 평균 급여는  %.1f 만원입니다." , counta , avgPay);
